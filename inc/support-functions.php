...
// Post Category
function creativeagency_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'creativeagency' ) . '</span></li>';
...
// Blog categories
function creativeagency_featured_post_cat(){
...
// Post Tags
function creativeagency_post_tags() {
...
// social media
if ( ! function_exists( 'creativeagency_social' ) ) {
	function creativeagency_social( $args = array() ) {
		...
		$url = creativeagency_opt( 'creativeagency_social_url' );
...
// header cart count
function creativeagency_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( CREATIVE_AGENCY_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'creativeagency' ); ?>">
...
function creativeagency_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'creativeagency_contact7_form_content', 10, 2 );
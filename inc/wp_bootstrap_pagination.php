...
function creativeagency_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'creativeagency' ),
        'next_string'     => esc_html__( '&raquo;', 'creativeagency' ),
        'before_output'   => '<div class="creativeagency-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'creativeagency_pagination_defaults', $defaults )
    );
